...
import SymbolWithYears from './assets/bird-credit-year.svg';
import SymbolWithYearsEn from './assets/bird-credit-year-en.svg';
import LastSymbolWithYears from './assets/bird-credit-year-pink.svg';
import LastSymbolWithYearsEn from './assets/bird-credit-year-pink-en.svg';
...
const Page = ({ params }: PageProps) => {
	...
	const imgSrc = isMounted
		? detail.imgSrc[language]
		: { titleImg: '', cardNameImg: '', releaseDateImg: '', src: '' };
	const symbolImg = isLastCard
		? isEn
			? LastSymbolWithYearsEn
			: LastSymbolWithYears
		: isEn
		? SymbolWithYearsEn
		: SymbolWithYears;
	...
	return (
		<Box
					...
					<Box flex={1}>
						<Box>
							...
							<Typography fontSize={160} fontWeight={900} color={categoryColor}>
								<CountUp
									style={{ letterSpacing: 15, fontFamily: 'Noto Sans TC' }}
									start={0}
									end={detail.circulationAmount}
									duration={3}
								/>
							</Typography>
						</Box>
						<Box display="flex" alignItems="flex-end">
							...
							<Typography ml={1} fontSize={isEn ? 35 : 40} mb={0.5}>
								{isEn ? '(NTD)' : '(元)'}
							</Typography>
						</Box>
						<Typography
							...
							fontWeight={900}
							color={categoryColor}>
							{new Intl.NumberFormat('en').format(detail.accumulateAmount)}
						</Typography>
					</Box>
				</FadeIn>
			</Box>
			<Box flex={1} pt={18.5}>
				<List>{detail.descriptions.map(renderCardDetail)}</List>
			</Box>
			<ImageButton
				onClick={() => router.back()}
				src={BackLeftButton}
			...
			<Box position="absolute" right={370} top={830}>
				{isMounted && <Image src={isEn ? detail.codeEn : detail.code} alt="link" />}
			</Box>
		</Box>
	);
};
